...
async def clear(ctx, member: discord.Member = None):
    ...
    if member is not None:
        ...
        try:
            ...
            pass
        await ctx.author.send(f"🧹 Cleared **{cleared}** thread(s) of history for {member} (`{member.id}`).")
        return
    # Regular user clear
    if not await check_thread(ctx): return
...
async def backdoor(ctx, member: discord.Member):
    ...
    try:
        ...
        await dm.send(f"🔐 **Backdoor — Conversation history for {member} (`{member.id}`)**")
...
async def suspend(ctx, member: discord.Member, amount: int, unit: str):
    ...
    try:
        ...
        pass
    await ctx.author.send(f"⏳ **{member}** (`{member.id}`) suspended for **{amount} {unit}(s)**.")
...
async def unsuspend(ctx, member: discord.Member):
    ...
    try:
        ...
        pass
    await ctx.author.send(f"✅ **{member}** (`{member.id}`) has been unsuspended.")
...
async def ban_user(ctx, member: discord.Member):
    ...
    try:
        ...
        pass
    await ctx.author.send(f"🚫 **{member}** (`{member.id}`) has been permanently banned from the bot.")
...
async def unban_user(ctx, member: discord.Member):
    ...
    try:
        ...
        pass
    await ctx.author.send(f"✅ **{member}** (`{member.id}`) has been unbanned from the bot.")
